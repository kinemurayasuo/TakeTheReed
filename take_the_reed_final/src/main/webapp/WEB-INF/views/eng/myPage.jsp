	...
	<c:if test="${not empty userList }"> 
	  <c:forEach items ="${userList }" var="user">
		<table style="margin-left: auto; margin-right: auto;" class="checkout__input">
	        ...
	        <tr>
	        	<td colspan="2">
	        		<input type="button" value="Change Personal Information" style="background-color:#ADFA7E; color:black; font-weight:bold; height:37pt; font-family:'IBM Plex Sans KR';  position:relative; top:15px; width:180pt;"
								onclick="location.href='/eng/pwchk'">
				</td>
				<td></td>
			</tr>	
	   </table>
	 ...
	 <c:if test="${not empty userList1 }"> 
	  <c:forEach items ="${userList1 }" var="user1">
		<table style="margin-left: auto; margin-right: auto;">
		    <h5 style="color:red; font-weight:bold; text-align:center;">In case of social Login, It cannot change a  Information.</h5>
	 		<tr>
	 			<td>SOCIAL ID</td>
	 			<td>${user1.userId }</td>
	        </tr>
	        ...
	        <tr>
	            <td>SOCIAL SUBSCRIPTION STATUS</td>
	            <td></td>
	        </tr>
	    </table>
	  </c:forEach>
	...
	<c:if test="${not empty pastList }">
		<c:forEach items ="${pastList }" var="past" varStatus="status">
			<div style="display:inline-block; text-align:center; margin: 0 auto; width:200px">
				...
				<ul> ${past.bookName }</ul>
				<ul>Views:${past.bookCk } &nbsp;&nbsp; Ratings:${past.bookStar }</ul>
			</div>
		</c:forEach>
	</c:if>
   </form>
